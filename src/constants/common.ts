export const LIMIT_PAGE = 16 as const
export const DAY_SAVE_COOKIE = 5 as const
export const TIME_CLOSE_MODAL_SUCCESS = 2400 as const
export const MAX_FILE_SIZE = 5000000 as const
export const APP_NAME = 'Kitten world' as const
export const ROLE = 'kitten_world_role' as const
export const KEY_TOKEN_COOKIE = 'kitten_world' as const
export const KEY_DATA_USERS_COOKIE = 'kitten_world_user' as const
export const LIMIT_ROOMS = 9 as const
export const COUNT_MESSAGE = 50 as const

export const ROLE_APP = {
  USER: 'R3',
  ADMIN: 'R1',
} as const
